...
AND (:transaction_type = 'A' OR :transaction_type = 'U') THEN
    ...
    IF EXISTS (
        ...
        WHERE T1."DocEntry" = :list_of_cols_val_tab_del
          AND IFNULL(T0."U_MOTIVO_NCND", 0) IN (1, 2, 4, 7, 8, 14, 16)
          AND T1."NoInvtryMv" <> 'N'
    ) THEN
        error := 1;
        error_message :=
        'Para este motivo de nota de crédito, todas las líneas deben tener devolución de cantidad y ser aprobadas por gerencia de logística';
    END IF;
    ...
    IF EXISTS (
        ...
        INNER JOIN "ORIN" T0 ON T0."DocEntry" = T1."DocEntry"
        WHERE T1."DocEntry" = :list_of_cols_val_tab_del
          AND IFNULL(T0."U_MOTIVO_NCND", 0) NOT IN (1, 2, 4, 7, 8, 14, 16)
          AND T1."NoInvtryMv" <> 'Y'
    ) THEN
        error := 1;
        error_message :=
        'Para este motivo de nota de crédito, no debe existir devolución de cantidad en las líneas';
    END IF;
